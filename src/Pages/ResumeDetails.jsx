...
const navigate = useNavigate()
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                setValue('personalDetails', data.personalDetails);
                setValue('summary', data.summary);
                setValue('education', data.education);
                setValue('professionalExperience', data.professionalExperience);
    ...
    return (
        <div className='md:flex gap-x-8 mx-16 pt-10 '>
            <div  onClick={() => handleDelete(id)} className='fixed bottom-10 right-10 bg-red-700 rounded-full p-5 cursor-pointer'>
   ...
   <p className='bg-red-300 p-1 rounded-full' onClick={() => handleAccordion('projects')}>{isOpen.projects ?  <BiSolidUpArrow/> : <BiSolidDownArrow />}</p>
    ...
    {skillFields.map((field, index) => {
        ...
        <div className="flex justify-center">
            <button
            type="submit"
            className="px-12 py-2 bg-blue-600 text-white font-semibold rounded-md"
            >
            Save
            </button>
        </div>
